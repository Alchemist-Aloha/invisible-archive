...
## 2024-05-24 - [Optimize Frontend Path Encoding]
**Learning:** In the frontend codebase, `cleanPath.split('/').map(encodeURIComponent).join('/')` causes unnecessary array allocations and string copies.
**Action:** Replace `split('/').map(...).join('/')` with `encodeURIComponent(path).replaceAll('%2F', '/')` to avoid these allocations and reduce Garbage Collection overhead.
