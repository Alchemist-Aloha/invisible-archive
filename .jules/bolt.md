...
## 2024-03-24 - Zero-Allocation Path Joining in Tight Loops
**Learning:** Using `filepath.Join` inside large O(N) loops (like parsing every entry in a ZIP file) introduces massive GC overhead due to its implicit calls to `filepath.Clean` which heavily allocates strings.
**Action:** In performance-critical loops where the input path format is already known/constrained (like clean ZIP entry paths), use direct string concatenation carefully handling slashes, avoiding `filepath.Join` entirely.
